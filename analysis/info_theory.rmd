...
-   you are always using the same notation conventions, such as: p(x) vs P(x), p(xnew) vs p(x'), data as x or as y, and so on.

-   state the rules of probability or the log identities when needed.
...
-   <div>
    ...
    1.  why are we interested in prediction accuracy? (good paragraph from gelman).
...
-   <div>
    ...
    3.  or 2. how do we measure predictive accuracy (general probability scoring rules, then logscores)

    </div>

-   <div>

    4.  (maybe in a box) what is IC doing? (good shit from gelman and from lambert).

    </div>

-   <div>

    5.  In the case of a hierarchical model, What are we trying to predict? in group or another group? differences in the posterior predictive distribution and why we are not bothered by the non-uniqueness of the posterior predictive distribution.
...
<!--# seeing what aurelion gereon said about kl div, that is is equal to the cross entropy minus the entropy, it seems like there's a minus sign, cuz the first term is the entropy of the true distribution (but missing a minus sign cuz that form of entropy has a negative due t the log) and the second term is the cross entropy, which is also negative and thus there should be a plus sign due to the double minus. Seems like its the kl divergence given by the entropy minus the cross entropy but all multiplied by -1 to invert the signs of each term. Does this have anything to do with the asymmetry of the kl divergence?  -->

The first term, $\int f(x^{new})\ log \ f(x^{new})dx^{new}$ <!--# isnt there a minus sign missing at the beggining? or is it that the second term "should" come first and there's a minus minus sign (thus plus sign) to the first term? maybe its related to the mathmatical definition of cross-entropy and its relation with entropy and KL divergence. -->, is essentially a constant because it only depends on $f(x^{new})$ and so that doesn't tell us anything about how well our model fits the data (and there's nothing we can do about that term). Whereas the second term, $\int f(x^{new})\ log\ p(x^{new}| \pmb x)\ dx^{new}$ , contains $f(x^{new})$ (our unknown true data generation process) but also the posterior predictive distribution $p(x^{new} | \pmb{x})$ which depends on the model that we fit (so there's something we can do about this term). This second term is called the "Expected Log Predictive Density" (ELPD) <!--# "expected" cuz we are integrating with respect to xnew? check gelman -->. We want to minimize the KL divergence because when $D_{KL} = 0$, then $f(x^{new}) = p(x^{new} | \pmb{x})$ (that is, our approximation $p(x^{new} | \pmb{x})$ of the unknown true data generation process $f(x^{new})$ is correct). And the only way we can minimize the KL divergence is by maximizing the ELPD <!--# maximizing because of the minus sign -->.
...
(both prior and predictive distributions are valid probability distributions, so the area beneath the pdfs should integrate to 1)

<!--# need to explain why a joint probability of A and B conditioned on C equals the prob of A given B and C times the prob of B given C. (maybe you explained it in that damianos paper on HMMs). I think it was called the chain rule of probability or chain rule of joint probability)  -->

------------------------------------------------------------------------

From Adam Luise: Intuitively understanding the shannon entropy. <https://www.youtube.com/watch?v=0GCGaw0QOhA&t=1s>

The Shannon Entopy $H(X)$ measures the uncertainty associated to a probability distribution.

$$
H(X) = -\sum_{i} P(x_i)\ log\ P(X_i)
$$

Discussion of shannong entropy from the perspective of information transfer.
...
-   Maybe insert the word "GAP" where you know there's a gap in your explanation, and explain at some point that you will fill them up as you understand it more.

-   say: for a deeper dive YOU CAN CHECK stones book (not, you should check or I recommend, cuz I dont really have the knowledge to make such a statement).
